...
\end{equation}
i.e., the flow map describes the movement of the tracers from one point in time
to another mathematically. Generally, the flow map is as smooth as the
...
\parencite{farazmand2012computing}. Thus, it has two real, positive eigenvalues
...
\end{equation}
where, in the context of tracer advection, incompressibility is equivalent to
...
\emph{hyperbolic} and \emph{parabolic}. Roughly speaking, parabolic LCSs
outline cores of jet-like trajectories, elliptic LCSs describe vortex
boundaries, whereas hyperbolic LCSs illustrate overall attractive or
...
\textcite{haller2010variational} derived a variational LCS theory based on the
Cauchy-Green strain tensor, defined by~\cref{eq:cauchygreen}, from which the
aforementioned conditions follow. The immediately relevant parts of Haller's
theory are quoted in
...
rather than attracted towards, the material line. Lastly, requiring that the
...
line, rather than being stretched along with it due to shear stress.
\\
\begin{defn}
...
\cref{thm:lcsexistence} \parencite{farazmand2012computing}:
...
\begin{thm}[Sufficient and necessary conditions for LCSs in two-dimensional
    ...
    over the time interval $[t_{0},t_{0}+T]$. Then $\mathcal{M}(t)$ is a
    repelling LCS over $[t_{0},t_{0}+T]$ if and only if all the following hold
    for all initial conditions $\vct{x}_{0}\in\mathcal{M}(t_{0})$:
    \begin{subequations}
...
set of largest eigenvalues of the Cauchy-Green strain tensor field.
Component-wise, the Hessian matrix of a general, smooth, scalar-valued function
$f$ is defined as
\begin{equation}
...
accordance with with \cref{def:normalrepellence}.
Conditions~\eqref{eq:lcsexistence3} and \eqref{eq:lcsexistence4} suffice to
enforce that the normal repulsion rate attains a local extremum along the LCS,
relative to all nearby material lines. Lastly, condition
\eqref{eq:lcsexistence2} ensures that this is a strict local maximum.

\section[Finite-time Lyapunov exponent fields as predictors for LCSs]
{Finite-time Lyapunov exponent fields as predictors for\\\phantom{2.4 }LCSs}
\label{sec:ftle_fields_as_predictors_for_lcss}

Finite-time Lyapunov exponent (hereafter abbreviated to FTLE) fields provide
a measure of the extent to which particles that start out close to each other,
...
strain tensor field, evaluated at $\vct{x}_{0}$. The absolute value of the
integration time $T$ is taken because one generally can consider the evolution
of a system in either temporal direction. Analogously to
\cref{def:attractinglcs}, the attraction of nearby trajectories can be estimated
by FTLEs found by means of integration backwards in time.
...
expect hyperbolic LCSs to be found, but it does not represent the blueprint for
the actual LCSs of a system in general.
